...
可以。设置 → **锁屏 PIN** 启用 4-32 位 PIN，支持：

- 快捷键 `⌘L` / `Ctrl+L` 手动锁
- 闲置 30min / 1h / 2h 自动锁
- 「App 重开时默认锁定」开关

PIN 后端 **bcrypt 哈希**存，验证不走任何远端。全屏毛玻璃遮罩 + PIN 输入框解锁。详见 [使用技巧 → 锁屏保护](./ux#🔒-锁屏保护)。

::: warning 锁屏不是数据加密
这是"视觉锁"，防的是路过偷看；别人物理拿到你电脑或 `preferences.json` + `decrypted/` 仍能读。真防物理入侵用全盘加密（FileVault / BitLocker）。
:::
...
旧版本只把"上次的时间范围"存在浏览器 localStorage 里，所以**清缓存 / 换浏览器 / Docker 跨主机访问**就会被引导到 WelcomePage 重选。

现在后端会把时间范围写到 `preferences.json` 的 `default_init_from / default_init_to`，下次启动直接自动开索引，无需前端介入。如果你升级后第一次仍被引导到选时间页，正常选一次，之后就稳了。

详见 [索引与初始化 § 自动重新初始化](./indexing#自动重新初始化)。
