...
1. 首次点「启用」会让你设 PIN（4-32 位，字母 / 数字 / 符号都行），后端 **bcrypt 哈希**存到 `preferences.json`，不落明文
2. 自动锁屏闲置时间：从不 / 30 分钟 / 1 小时 / 2 小时
3. 「App 重开时默认锁定」勾上 → 每次启动都要 PIN
...
- 全屏 `z-index: 9999` 毛玻璃遮罩，底下内容看不见
- 绿色圆形 🔒 图标 + 「WeLink 已锁定」+ PIN 输入框 + 解锁按钮
- 锁屏状态只存 `sessionStorage`，浏览器关了再开默认**不锁**（除非勾了「启动即锁」）
...
桌面端（lg+）设置页左侧有一栏 sticky 目录，按 5 组分类列出所有 section：**通用**（录屏 / 显示）· **AI 模型**（LLM / Embedding / 记忆 / 生图 + Prompt 模板 + TTS）· **隐私与安全**（屏蔽 / 屏幕锁）· **数据**（多账号 / 备份 / 关系预测忽略 / 配置管理）· **系统**（服务配置 / 应用配置 / 移动配对 / 诊断 / 用量 / 关于）。滚动时当前 section 自动高亮。小屏走顶部横向"快速跳转"芯片栏。

顶部搜索框支持按关键词过滤 section — 输入"下载"只显示下载目录相关的几块，输入"LLM"只显示 LLM 配置。支持中英文关键词。
