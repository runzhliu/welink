...
侧边栏「实验室」Tab，集合了一组**好玩、能分享**的小工具。和"分析"路线不同，这一组以"卡片化、可截图、像 Spotify Wrapped 一样" 为目标。v0.2.0 一次开 5 个，到 v0.2.x 又陆续加到 12+ 个 —— 详见 [docs/labs.md](docs/labs.md)。
...
- 默认 provider：**火山方舟（豆包）即梦** `doubao-seedream-3-0-t2i-250415`，走 OpenAI 兼容的 `/images/generations`
- **本地 hash 缓存**：`sha256(provider|model|size|prompt)` → `~/.welink/ai_images/<hash>.png`。命中直接秒返，不重复调 API
- **同源代理**：前端用 `GET /api/image/cache/:hash` 拿图（火山返回的临时 URL 24h 过期；html-to-image 导出分享卡也要求同源）
- Demo 模式写 SVG 占位图，不调真 provider
- 设置页可换 provider / Base URL / Model / 关闭整个生图能力
