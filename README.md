...
侧边栏「实验室」Tab，集合了一组**好玩、能分享**的小工具。和"分析"路线不同，这一组以"卡片化、可截图、像 Spotify Wrapped 一样" 为目标。从 v0.2.0 首发 5 个起持续扩充，现已累计 **21 个**（聊天 DNA / 高光瞬间 / 灵魂提问机 / 平行宇宙 / 关系星图 / 群金句榜 / 语言进化史 / 聊天地图 / 人情债 / 断联预警 / 群聊 Wrapped / 反向语义搜索 / AI 虚拟群聊 / 群语料 ROI / 关系考古 / 健康日记 / 暧昧探测 / 回复速度榜 / 主动指数榜 / 社交圈流动榜 / 话题图谱）—— 详见 [docs/labs.md](docs/labs.md)。会调 LLM 的 Lab 在 tab 上带紫色 **AI** 标。
...
> 每个 Lab 都支持**白底分享图导出**，footer 自带 `welink.click` + 日期，发朋友圈合适。
...
- **任务类型**：定时总结 / 待办挖掘 / 关键词盯梢 / 情绪变化 / 自定义 prompt
- **分析范围**：某个私聊 / 某个群 / 全部私聊 / 全部群聊
- **周期**：每天固定时间 / 每周几固定时间 / 每 N 小时间隔
- **收件箱**：跨任务汇总最近结果 + 未读数，可一键全部标记已读；每个任务也能看运行历史
...
- **👤 群友画像**：点群成员榜里任意成员（不限好友），弹出 TA 的群内专属画像 —— 群内排名 / 消息占比 / 活跃天数 / 深夜发言比例、24h 时段分布、周分布、近一年月度趋势、消息类型分布、TA 的高频词、最近发言样本；是好友的还能一键跳私聊看「完整分析」
- **🎬 群聊回放**：按最近 N 条 / 按日期范围加载群消息，7 档倍速（实时 / 2× / 5× / 10× / 50× / 100× / 200×）按真实时间间隔回放，连续同发言人合并头像 + 日期分割线。支持**全屏回放**（`Esc` 退出）、**智能节奏**（冷场 >5 分钟直接跳过、发言密集按内容长度放慢到能读完）、快进步长可切 3 / 5 / 10 条；快捷键 `空格` 播放/暂停、`→` 快进
...
| **Tab 快捷切换** | `⌘1` .. `⌘9` | 首页 / 今日简报 / 统计 / 联系人 / 群聊 / 搜索 / 日历 / 纪念日 / Skills（定时任务、AI 画廊等 tab 走侧边栏点选，无 ⌘ 绑定）|
