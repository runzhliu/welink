...
func startApp() {
	...
	// Show a branded loading screen immediately (light background, visible on all displays)
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  <div class="error" id="error"></div>
	...
	// Wait for server in background, update status, then navigate
	go func() {
		port := <-serverPortCh
		...
		if !ok {
			w.Dispatch(func() {
				...
				w.Eval(`var e = document.getElementById('error'); e.style.display='block'; e.textContent = '服务启动超时（30秒），请检查 decrypted/ 数据目录是否存在。可在设置中重新选择数据目录后重启。'`)
			})
			// Still try to navigate — the setup page can handle missing data
