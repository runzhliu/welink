        ...
        <div className="flex-1 space-y-1.5">
          {data.map((entry) => (
            <div key={entry.name} className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-gray-600 font-semibold flex-1">{entry.name}</span>
              {entry.count > 0 && (
                <span className="text-[10px] text-gray-400">{entry.count.toLocaleString()}</span>
              )}
              <span className="text-xs font-black text-gray-700 w-8 text-right">{entry.pct}%</span>
            </div>
          ))}
        </div>
      </div>
