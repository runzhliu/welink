...
import { Sparkles, Download, Trash2, Loader2, Package, User, Users, Bot } from 'lucide-react';
...
export const SkillsView: React.FC = () => {
  ...
  const loadSkills = useCallback(async () => {
    ...
    try {
      ...
      setToast({ type: 'error', text: `加载失败: ${(e as Error).message}` });
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <div className="dk-card bg-white dk-border border border-gray-100 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-gray-100 dark:divide-white/5">
            {skills.map(rec => {
              ...
              return (
                <div key={rec.id} className="grid sm:grid-cols-[1fr_auto_auto_auto_auto_auto] gap-4 px-5 py-4 items-center hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                  {/* 目标名 + 文件名 */}
                  ...
                  <div className="w-20 flex items-center justify-end gap-1">
                    <button
                      ...
                      className="p-1.5 rounded-lg text-gray-400 hover:text-[#07c160] hover:bg-[#07c160]/10 transition-colors disabled:opacity-50"
                      title="重新下载"
                    >
